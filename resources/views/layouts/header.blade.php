<div class="nav-shell position-absolute top-0 start-50 translate-middle-x w-100 px-3 z-3"
    style="max-width: 1450px; margin-top: 16px;">
    <nav class="navbar navbar-expand-lg glass-card rounded-4 px-3 py-2">
        <div class="container-fluid p-0">
            <a class="navbar-brand d-flex align-items-center gap-2 m-0" href="{{ url('/') }}">
                <div class="logo-box rounded-3 text-white d-grid place-items-center"
                ...
                <div>
                    ...
                    <small class="text-secondary fw-semibold" style="font-size: 9px; letter-spacing: 0.1px;">Study
                        Library SaaS</small>
                </div>
            </a>
            ...
            <div class="collapse navbar-collapse" id="mainNav">
                ...
                @auth
                    ...
                    <span class="d-flex align-items-center px-2 small fw-bold text-dark">
                        <i class="fa-solid fa-user-circle text-primary me-2"></i>
                        {{ Auth::user()->name }}
                    </span>

                    <a href="/library-dashboard" class="btn btn-primary fw-bold rounded-3 px-2 py-1">
                        Dashboard
                    </a>
                    <form method="POST" action="{{ route('library.logout') }}" class="m-0">
                        @csrf
                        <button type="submit" class="btn btn-outline-danger fw-bold rounded-3 px-2 py-1 mx-1">
                            Logout
                        </button>
                    </form>
                @else
